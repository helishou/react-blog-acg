...
class Category extends PureComponent {
  ...
  render() {
    ...
    return (
      <Itemlayout timg={this.state.timg} title={this.setCategory(category)}>
        <MainWrapper>
          <header className="page-header">
            <h1 className="page-title">{`分类 “${this.setCategory(
              category
            )}” 下的文章`}</h1>
          </header>
        ...
        if (res.code === 0) {
          ...
          for (let i = 0; i < data.length; i++) {
            arr.push({
              ...
              title: data[i].title,
              thumbnail:
                data[i].img_url || Img[this.getrand(0, Img.length - 1)].img,
